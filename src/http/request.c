...
struct http_reqconn {
	...
	http_req_bodyh *req_bodyh;/**< Handler for the request body          */
	uint64_t req_body_len;    /**< Size of body if request handler used  */
...
static size_t req_body_handler(uint8_t **pbuf, void *arg)
{
	struct http_reqconn *conn = arg;
	size_t len = 0;
	...
	*pbuf = NULL;
	...
	if (conn->req_bodyh) {
		len = conn->req_bodyh(pbuf, conn->arg);
	}
	else if (conn->body) {
		...
		*pbuf = mem_alloc(len, NULL);
		if (!*pbuf) {
			DEBUG_WARNING("error allocating %d\n", len);
			return -2;
		}

		memcpy(*pbuf, mbuf_buf(conn->body), len);
...
static int send_req(struct http_reqconn *conn, const struct pl *auth)
{
	...
	if (conn->body || conn->req_bodyh) {
		...
		if (conn->req_body_len >
				(uint64_t)http_client_get_bufsize_max(
				conn->client))
			mbuf_printf(clbuf, "Expect: 100-continue\r\n");
...
int http_reqconn_set_body(struct http_reqconn *conn, struct mbuf *body)
{
	...
	conn->req_body_len = mbuf_get_left(conn->body);
