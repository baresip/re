...
static int http_verify_handler(int ok, X509_STORE_CTX *ctx, void *arg)
{
	(void) ctx;
	struct http_verify_msg_d *d = arg;
	...
	struct http_msg *msg)
{
	...
	else if (!conn->sock->verify_msg_h)
		return HTTPS_MSG_OK;

	res = conn->sock->verify_msg_h(conn, msg, conn->sock->arg);
	...
	if (res == HTTPS_MSG_REQUEST_CERT) {
		...
		err = tls_set_conn_verify_client_handler(http_conn_tls(conn),
		...
		if (!strcmp(tls_conn_version(http_conn_tls(conn)),
				"TLSv1.3")) {
			err = tls_conn_verify_client_post_handshake(
					http_conn_tls(conn));
			if (err) {
				res = HTTPS_MSG_IGNORE;
				goto out;
 ...
 * @param verify_msg_h 	Verify handler called before the https request
